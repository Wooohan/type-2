...
const DashboardView: React.FC = () => {
  ...
  // Simulate seeing the 'ingress' operations that Zayn saw in the Atlas UI
  useEffect(() => {
  ...
  const stats = [
    ...
    { label: 'Atlas Driver Ping', value: "18ms", icon: Clock, color: 'text-purple-600', bg: 'bg-purple-50' },
    { label: 'Successful Persists', value: (dashboardStats.resolvedToday + 1200).toString(), icon: CheckCircle2, color: 'text-green-600', bg: 'bg-green-50' },
    { label: 'Op Throughput', value: "0.4/s", icon: Zap, color: 'text-amber-600', bg: 'bg-amber-50' },
  ];
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-widest transition-colors duration-500 ${
              ...
              Atlas: Zayn's Cluster0 {dbStatus.toUpperCase()}
            </div>
            ...
            {pulse && (
               <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-600 text-white text-[9px] font-black uppercase tracking-widest animate-in fade-in zoom-in-95">
                 <Activity size={10} /> Live Ingress
               </div>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-10 rounded-[48px] border border-slate-100 shadow-sm relative overflow-hidden">
          <div className="flex items-center justify-between mb-10 relative z-10">
             <div className="flex items-center gap-3">
                ...
                <h3 className="text-2xl font-black text-slate-900 tracking-tight">Database Traffic (Sync)</h3>
             </div>
             <div className="px-4 py-2 bg-slate-50 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-400 border border-slate-100">
               Last Pull: {dbStatus === 'connected' ? 'Active' : 'Awaiting Connection'}
             </div>
          </div>
        ...
        <div className="bg-slate-900 p-12 rounded-[56px] text-white shadow-2xl relative overflow-hidden group">
           ...
           <div className="flex flex-col h-full relative z-10">
             ...
             <h3 className="text-3xl font-black mb-4 tracking-tight">Zayn's Security</h3>
             <p className="text-slate-400 text-base leading-relaxed mb-12">Atlas Node Driver is active. The movement you saw in the Atlas charts is the heartbeat of this terminal connecting to your collection.</p>
             
             <div className="space-y-6 mt-auto">
                <div className="flex items-center justify-between text-[11px] font-black text-blue-400 uppercase tracking-[0.2em]">
                  <span>Atlas Heartbeat</span>
                  <span className={dbStatus === 'connected' ? 'text-emerald-400' : 'text-amber-400'}>
                    {dbStatus === 'connected' ? '100% HEALTHY' : 'NEGOTIATING'}
                  </span>
                </div>
                <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                  <div className={`h-full transition-all duration-1000 ${dbStatus === 'connected' ? 'bg-emerald-500 w-full' : 'bg-blue-500 w-1/2 animate-pulse'}`} />
                </div>
                <button 
